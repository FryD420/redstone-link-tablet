...
/**
 * The list-mode signal row (plaque, icon chip, name, per-type control),
...
final class SignalRowPainter {
     ...
     */
    static String paint(GuiGraphics graphics, Font font, ScreenTheme theme, Signal signal,
                        int x, int y, int w, boolean hovered, boolean held) {
        boolean lit = signal.active() || held;
        Chrome.plaque(graphics, x, y, w, TabletScreen.ROW_HEIGHT, hovered ? theme.rowBgHover : theme.rowBg);

        // Colored icon chip
        graphics.fill(x + 4, y + 4, x + 20, y + 20, signal.color() | 0xFF000000);
        graphics.renderItem(signal.iconStack(), x + 4, y + 4);

        // Name (leave room for chip + glyph gap + switch/track + count tag)
        String countTag = signal.frequencies().size() > 1 ? " x" + signal.frequencies().size() : "";
        int tagWidth = countTag.isEmpty() ? 0 : font.width(countTag);
        // Sliders reserve extra room for the numeric level readout
        int controlW = signal.slider()
                ? TabletScreen.LIST_SLIDER_W + font.width("15") + 4
                : TabletScreen.SWITCH_W;
        String name = TextFit.ellipsize(font, signal.name(), w - 24 - controlW - 24 - tagWidth);
        ...
        if (signal.slider()) {
            ...
            int knobX = tx0 + Math.round((tx1 - tx0 - 4) * signal.fillFraction());
        ...
        if (signal.timed()) {
            ...
            // Toggle switch
            int track = signal.active() ? theme.accentDim : theme.switchOff;
            graphics.fill(sx, sy, sx + TabletScreen.SWITCH_W, sy + TabletScreen.SWITCH_H, track);
            int knobX = signal.active() ? sx + TabletScreen.SWITCH_W - 10 : sx + 2;
            graphics.fill(knobX, sy + 2, knobX + 8, sy + TabletScreen.SWITCH_H - 2,
                    signal.active() ? theme.accent : theme.textMuted);
        }
        return name;
    }

    private SignalRowPainter() {
    }
}
