...
2. **Sequenced-assembly tablet recipe (IMPLEMENTED 2026-07-24,
   awaiting user's in-game test)** — Option A, DEPLOYER-ONLY (user
   decision; old crafting-table tablet.json DELETED). Craft a brass
   `tablet_case` ("P P"/"PPP" sheets), Deployers add `logic_board`
   (link + linked controller + electron tube), `clockwork_cell` (iron
   nugget + rose quartz + cogwheel), then `quartz_display`
   (2 polished rose quartz + tinted glass) LAST — screen goes on at
   the end (user call 2026-07-25; incomplete texture shows board +
   cell). `recipe/sequenced_assembly/
   tablet.json`, transitional `incomplete_tablet` extends Create's
   SequencedAssemblyItem. 5 new items → BREAKS PAIRING with 1.8.x,
   ships as 1.9.0. Textures are string-map pixel art generated by a
   scratchpad TexGen (brass/quartz shades sampled from Create's own
   sprites); regen means rewriting the tool, or just edit the PNGs.
   Boot-checked on the dev server: recipes parse, 0 errors. BEFORE
   RELEASE: user tests the actual deployer line in-game; update
   DESCRIPTION.md "recipe" section + recipe.png (lines ~184-194 still
   show the old crafting grid) and both listings' images.
3. **App links — apps toggling other apps (scoped 2026-07-25)** —
   ...
   extend the HAND-ROLLED stream codec) → registrar bump, so ship
   INSIDE 1.9.0 to share the items' pairing break if possible.
